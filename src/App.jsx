...
function App() {
  ...
  const [myProfile, setMyProfile] = useState({
    full_name: '', blood_group: '', phone: '', location: '', address: '', bio: '', status: 'Active Donor', avatar_url: ''
  });
        ...
        <form onSubmit={async (e) => {
          ...
          if (authView === 'login') {
            ...
            const { error } = await supabase.auth.signUp({ 
              email, password: pass, 
              options: { data: { full_name: fd.get('name'), blood_group: fd.get('bgroup') } } 
            });
      ...
      <main className="content-container">
        ...
        {activeTab === 'home' && (
          <div className="home-view animate-fade">
            ...
            <div className="row-2" style={{marginTop:'2rem'}}>
               <button className="btn-primary" onClick={() => setActiveTab('request')}>জরুরী আবেদন</button>
               <button className="glass" style={{padding:'1.1rem'}} onClick={() => setActiveTab('donors')}>রক্তদাতা খুঁজুন</button>
            </div>
          </div>
        )}
          ...
          <div className="profile-view animate-fade">
            ...
            <div className="glass form-container" style={{marginTop:'2rem', padding:'2.5rem'}}>
              <form onSubmit={saveProfile} className="pro-form">
                <div className="input-group"><label>পুরো নাম</label><input name="name" defaultValue={myProfile.full_name} required /></div>
                <div className="row-2">
                  <div className="input-group"><label>রক্তের গ্রুপ</label><select name="bgroup" defaultValue={myProfile.blood_group}><option value="">নির্বাচন করুন</option>{bloodGroups.map(bg => <option key={bg} value={bg}>{bg}</option>)}</select></div>
                  <div className="input-group"><label>মোবাইল নম্বর</label><input name="phone" defaultValue={myProfile.phone} placeholder="০১৭XXXXXXXX" required /></div>
                </div>
                <div className="row-2">
                  <div className="input-group"><label>শহর/এলাকা</label><input name="loc" defaultValue={myProfile.location} placeholder="ঢাকা" /></div>
                  <div className="input-group"><label>ডোনার স্ট্যাটাস</label><select name="status" defaultValue={myProfile.status}><option value="Active Donor">রক্ত দিতে প্রস্তুত</option><option value="Unavailable">ব্যস্ত আছি</option></select></div>
                </div>
            ...
            <div className="pro-grid">
              {requests.filter(r => r.status !== 'completed').map(req => (
                <div key={req.id} className="pro-req-card glass">
                  <div className="req-header"><div className="blood-badge">{req.blood_group}</div><div className="posted-by">{req.posted_by}</div></div>
                  <h3>{req.patient_name}</h3><p>📍 {req.location}</p><p>📞 {req.phone}</p>
        ...
        {activeTab === 'donors' && (
          <div className="donor-view animate-fade">
            ...
            <div className="filter-area">
              <input placeholder="খুঁজুন..." className="glass search-input" onChange={(e) => setSearchQuery(e.target.value.toLowerCase())} />
              <select className="glass filter-select" onChange={(e) => setFilterBG(e.target.value)}><option value="All">সব গ্রুপ</option>{bloodGroups.map(bg => <option key={bg} value={bg}>{bg}</option>)}</select>
            </div>
            <div className="pro-grid">
              {profiles.filter(p => (p.full_name?.toLowerCase().includes(searchQuery) || p.location?.toLowerCase().includes(searchQuery)) && (filterBG === 'All' || p.blood_group === filterBG)).map(p => (
                <div key={p.id} className="pro-donor-card glass" onClick={() => setSelectedUser(p)}>
                  <div className="donor-header"><img src={p.avatar_url || `https://ui-avatars.com/api/?name=${p.full_name}&background=random`} alt="Avatar" className="donor-avatar" /><div className="donor-blood-tag">{p.blood_group || '?'}</div></div>
                  <h3>{p.full_name}</h3><p>📍 {p.location || 'Unknown'}</p><span className={`status-dot ${p.status === 'Active Donor' ? 'active' : 'busy'}`}></span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="modal-overlay" onClick={() => setSelectedUser(null)}>
            <div className="pro-modal glass animate-fade" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-details">
                <p>📍 <strong>এলাকা:</strong> {selectedUser.location}</p><p>🏠 <strong>ঠিকানা:</strong> {selectedUser.address}</p><p>📞 <strong>ফোন:</strong> {selectedUser.phone}</p><p>📝 <strong>বায়ো:</strong> {selectedUser.bio}</p>
              </div>
        ...
        {activeTab === 'request' && (
          <div className="request-view animate-fade">
             <div className="glass form-container" style={{padding:'2.5rem'}}>
              ...
              <form onSubmit={async (e) => {
                ...
                const { error } = await supabase.from('blood_requests').insert([{ 
                  patient_name: fd.get('pname'), blood_group: fd.get('bgroup'), location: fd.get('loc'), phone: fd.get('phone'), posted_by: session.user.email, status: 'pending' 
                }]);
                if(!error) { alert('আবেদন পোস্ট হয়েছে! 🩸'); setActiveTab('live'); } else alert(error.message);
                setLoading(false);
              }}>
